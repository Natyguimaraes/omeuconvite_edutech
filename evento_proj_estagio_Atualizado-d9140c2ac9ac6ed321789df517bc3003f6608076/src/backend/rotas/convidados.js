...
router.post("/:convidadoId/acompanhantes", async (req, res) => {
  ...
  try {
    const [result] = await conexao.execute(
      "INSERT INTO acompanhantes (convidado_id, nome, telefone, email) VALUES (?, ?, ?, ?)",
      [convidadoId, nome, telefone, email]
    );
    ...
    res.status(500).json({ erro: "Erro ao adicionar acompanhante" });
  }
});

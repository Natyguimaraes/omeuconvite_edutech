...
function CadastroEventos() {
  ...
  const [data_gerar_qrcode, setdata_gerar_qrcode] = useState("");
  ...
  // Função para formatar datas para o banco de dados (mantém o formato ISO completo)
  ...
  const formatarParaExibicao = (dataString) => {
    if (!dataString) return "";
    
    const dateObj = new Date(dataString);
    if (isNaN(dateObj.getTime())) return "Data inválida";
    
    // Compensação para exibição
    dateObj.setHours(dateObj.getHours() + 3);
  ...
  const handleCadastro = async () => {
    ...
    // Formatação das datas para o banco
    const dataEventoParaBanco = formatarParaBanco(dataEvento);
    const dataQrCodeParaBanco = formatarParaBanco(data_gerar_qrcode);

    if (!dataEventoParaBanco || !dataQrCodeParaBanco) {
      setError("Datas fornecidas são inválidas.");
      toast.error("Datas fornecidas são inválidas.");
      setIsSubmitting(false);
      return;
    }

    // Comparação de datas (considerando UTC)
    const dataQrCodeObj = new Date(dataQrCodeParaBanco);
    const dataEventoObj = new Date(dataEventoParaBanco);

    if (dataQrCodeObj >= dataEventoObj) {
    ...
    formData.append("data_evento", dataEventoParaBanco);
    formData.append("data_gerar_qrcode", dataQrCodeParaBanco);
    formData.append("local", local);
    formData.append("mensagem_whatsapp", mensagem_whatsapp);
    formData.append("tipo", tipo);
    formData.append("administrador_id", adminIdNumber);

    // Debug - mostre as datas formatadas
    console.log("Datas sendo enviadas:", {
      data_evento: dataEventoParaBanco,
      data_gerar_qrcode: dataQrCodeParaBanco,
      data_evento_exibicao: formatarParaExibicao(dataEventoParaBanco),
      data_qrcode_exibicao: formatarParaExibicao(dataQrCodeParaBanco)
    });
          ...
          <div className="backdrop-blur-md bg-white/80 rounded-3xl overflow-hidden border border-white/30 shadow-xl">
            <div className="p-8 sm:p-10">
              <div className="text-center mb-10">
                ...
                <h1 className="text-3xl font-extrabold tracking-tight text-gray-800 mb-2">Criar Novo Evento</h1>
                <p className="text-gray-600 max-w-lg mx-auto">Preencha os detalhes abaixo para criar um evento memorável</p>
              </div>
              ...
              <div className="space-y-6">
                    ...
                    <div className="space-y-1 text-center">
                      <div className="flex text-sm text-gray-600 justify-center">
                        <label className="relative cursor-pointer rounded-md font-medium text-indigo-600 hover:text-indigo-500 focus-within:outline-none">
                          ...
                          <input type="file" className="sr-only" accept="image/jpeg, image/png, image/gif" onChange={handleImageChange} />
                        </label>
                      </div>
                      <p className="text-xs text-gray-500">PNG, JPG, GIF até 10MB</p>
                      {imagemPreview && (
                        <div className="mt-4">
                          <img src={imagemPreview} alt="Pré-visualização" className="mx-auto h-32 object-cover rounded-md" onLoad={() => URL.revokeObjectURL(imagemPreview)} />
                          <p className="text-sm text-gray-900 mt-2 truncate px-2">{imagem_evento.name}</p>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                {/* Data para liberação de credencial */}
                <div className="space-y-2">
                  <label className="flex items-center text-sm font-medium text-gray-700">
                  ...
                  <input
                    type="datetime-local"
                    value={data_gerar_qrcode}
                    onChange={(e) => setdata_gerar_qrcode(e.target.value)}
                    className="w-full px-4 py-3 rounded-xl bg-white/90 border border-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-300 transition-all"
                    required
                  />
                  <p className="text-xs text-gray-500">Defina quando as credenciais estarão disponíveis para os convidados</p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    className="w-full px-4 py-3 rounded-xl bg-white/90 border border-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-300 transition-all resize-none"
                  />
                  <p className="text-xs text-gray-500">Esta mensagem será usada como texto pré-definido quando os convidados clicarem no botão do WhatsApp</p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <option value="Aniversário de adolescente">
                      Aniversário de adolescente
                    </option>
                    <option value="Aniversário de adulto">
                      Aniversário de adulto
                    </option>
                    <option value="Aniversário de debutante">
                      Aniversário de debutante
                    </option>
                    <option value="Aniversário de pet">
                      Aniversário de pet
                    </option>
                    <option value="Aniversário infantil">
                      Aniversário infantil
                    </option>

                    {/* BODAS */}

                    <option value="Bodas de Algodão (1 ano)">
                      Bodas de Algodão (1 ano)
                    </option>
                    <option value="Bodas de Aço (11 anos)">
                      Bodas de Aço (11 anos)
                    </option>
                    <option value="Bodas de Barro (8 anos)">
                      Bodas de Barro (8 anos)
                    </option>
                    <option value="Bodas de Cerâmica (9 anos)">
                      Bodas de Cerâmica (9 anos)
                    </option>
                    <option value="Bodas de Coral (35 anos)">
                      Bodas de Coral (35 anos)
                    </option>
                    <option value="Bodas de Couro (3 anos)">
                      Bodas de Couro (3 anos)
                    </option>
                    <option value="Bodas de Cristal (15 anos)">
                      Bodas de Cristal (15 anos)
                    </option>
                    <option value="Bodas de Diamante (60 anos)">
                      Bodas de Diamante (60 anos)
                    </option>
                    <option value="Bodas de Esmeralda (40 anos)">
                      Bodas de Esmeralda (40 anos)
                    </option>
                    <option value="Bodas de Estanho (10 anos)">
                      Bodas de Estanho (10 anos)
                    </option>
                    <option value="Bodas de Lã (4 anos)">
                      Bodas de Lã (4 anos)
                    </option>
                    <option value="Bodas de Latão (7 anos)">
                      Bodas de Latão (7 anos)
                    </option>
                    <option value="Bodas de Madeira (5 anos)">
                      Bodas de Madeira (5 anos)
                    </option>
                    <option value="Bodas de Marfim (14 anos)">
                      Bodas de Marfim (14 anos)
                    </option>
                    <option value="Bodas de Ouro (50 anos)">
                      Bodas de Ouro (50 anos)
                    </option>
                    <option value="Bodas de Papel (2 anos)">
                      Bodas de Papel (2 anos)
                    </option>
                    <option value="Bodas de Pérola (30 anos)">
                      Bodas de Pérola (30 anos)
                    </option>
                    <option value="Bodas de Perfume (6 anos)">
                      Bodas de Perfume (6 anos)
                    </option>
                    <option value="Bodas de Platina (70 anos)">
                      Bodas de Platina (70 anos)
                    </option>
                    <option value="Bodas de Porcelana (20 anos)">
                      Bodas de Porcelana (20 anos)
                    </option>
                    <option value="Bodas de Prata (25 anos)">
                      Bodas de Prata (25 anos)
                    </option>
                    <option value="Bodas de Rendá (13 anos)">
                      Bodas de Rendá (13 anos)
                    </option>
                    <option value="Bodas de Rubi (45 anos)">
                      Bodas de Rubi (45 anos)
                    </option>
                    <option value="Bodas de Seda (12 anos)">
                    ...
                    <option value="Casamento Religioso">
                      Casamento Religioso
                    </option>
                    ...
                    <option value="Encontro de amigos">
                      Encontro de amigos
                    </option>
                    <option value="Encontro de negócios">
                      Encontro de negócios
                    </option>
                    <option value="Encontro familiar">Encontro familiar</option>
                    <option value="Evento beneficente">
                      Evento beneficente
                    </option>
                    ...
                    <option value="Lançamento de produto">
